...
def show_spectrum(name,xx,fObs,eObs,fExp,ebar,func1,file): 
    ...
    box1.plot(xx, fObs,color ="#b22222") #brick fire
    ...
    xtickname=['1200' ,   '1000' ,    '800'  ,   '700'  ,   '600']
    pollo=[54.9820,      46.8213,      31.9405,      21.2406 ,     7.28180]

    ax2.set_xlim(box1.get_xlim())
    ax2.spines['top'].set_color("#b22222") 
    ax2.set_xticklabels(xtickname,color ="#b22222")
    ax2.set_xlabel(r"wavelength[nm]",color ="#b22222")
    ax2.set_xticks(pollo)
    ax2.tick_params(axis='both',  which='major',direction='in', color ="#b22222",left=False, top=True, right=False, bottom=False, labelleft=False, labeltop=True, labelright=False, labelbottom=False)
    plt.savefig(file,dpi=1000)

##############

pdir = pathlib.Path('.').resolve()
ff1 = 'RP_QSO-v036a-DIOPS-1465-bspline.csv'
df1 = pd.read_csv(ff1)
